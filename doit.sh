...
if [ ! -f peer.pfx.pathlen.$YYYY.$MM.$DD.txt.gz ]; then
   ...
   done | ./pathlen.py | gzip -9 > peer.pfx.pathlen.$YYYY.$MM.$DD.txt.gz
fi
...
if [ ! -f stats.$YYYY.$MM.$DD.txt.gz ]; then
   ./dist-stats.py peer.pfx.pathlen.$YYYY.$MM.$DD.txt.gz $YYYY-$MM-$DD | gzip -9 > stats.$YYYY.$MM.$DD.txt.gz
fi
...
if [ ! -f improvements.$YYYY.$MM.$DD.txt.gz ]; then
   ...
   done | ./calc-dist-improvements.py peer.pfx.pathlen.$YYYY.$MM.$DD.txt.gz stats.$YYYY.$MM.$DD.txt.gz $THRES $YYYY-$MM-$DD | tee improvements.$YYYY.$MM.$DD.txt
fi
